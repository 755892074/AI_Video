...
import os, shutil, subprocess, argparse, imageio.v2 as imageio
...
def _find_font():
    for p in (r"C:/Windows/Fonts/simhei.ttf", r"C:/Windows/Fonts/msyh.ttc",
              r"/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc", r"/System/Library/Fonts/PingFang.ttc"):
        if os.path.exists(p):
            return p
    return r"C:/Windows/Fonts/simhei.ttf"  # 兜底

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--series", default="hp", help="镜头系列名(对应 shots/<series>)")
    ap.add_argument("--src", default=None, help="镜头mp4目录, 默认 shots/<series>/output")
    ap.add_argument("--work", default=None, help="输出工作目录, 默认 <src>/_concat")
    ap.add_argument("--srt", default=None, help="外部字幕srt路径(替代内置DIALOGUE)")
    args = ap.parse_args()
    SRC_DIR = args.src or os.path.join(ROOT, "shots", args.series, "output")
    WORK = args.work or os.path.join(SRC_DIR, "_concat")
