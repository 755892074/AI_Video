...
function makeHuman(opts) {
  ...
  // 手走路摆动 + 身体上下 bounce 暴露接口
  root.userData.walk = (phase, bounce) => {
    const s = Math.sin(phase), c = Math.cos(phase);
    legL.hip.rotation.x  =  s * 0.7;
    legR.hip.rotation.x  = -s * 0.7;
    legL.knee.rotation.x = Math.max(0, -c) * 0.9;   // 膝盖只在腿前摆时弯
    legR.knee.rotation.x = Math.max(0,  c) * 0.9;
    armL.shoulder.rotation.x = -s * 0.6;
    armR.shoulder.rotation.x =  s * 0.6;
    body.position.y = bounce;                         // 全身上下
    torso.rotation.z = s * 0.04;                      // 轻微左右倾
  };
  return root;
}
...
function frame() {
  ...
  const bounce = Math.abs(Math.sin(phase * 0.5)) * 0.05; // 上下 bounce
